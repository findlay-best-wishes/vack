...
inquirerInit()
	.then(() => {
		...
		console.log(
			'\n',
			task(
				`We're going to create new project named ${
					options.name
				} with ${framework}${options.withTs ? ' and typescript' : ''}\n`
			)
		);
		require('./run')(options);
...
function commanderInit() {
	...
	if (program.opts().ts) options.withTs = true;
	if (program.opts().vue && program.opts().react) {
		console.log('\n', error('do not use --vue and react at the same time'));
		process.exit(1);
	}
	if (program.opts().react) options.withReact = true;
	if (program.opts().vue) options.withReact = false;
}
...
function inquirerInit() {
	...
	if (typeof options.withReact !== 'boolean')
		questions.push({
			...
			type: 'confirm',
			message:
        'if you want to use react,please enter y.if you want to use vue,please enter n.'
		});
	if (!options.withTs)
		questions.push({
			name: 'withTs',
			type: 'confirm',
			message:
        'if you want to use typescript,please enter y. if not please enter n.'
		});
...
const usageGuide = (dir: string) => {
	...
	console.log(
		'\n',
		task(
			'now the project has been created successfully,you probably want to exec'
		)
	);
	console.log(`\ncd ${guide(dir)}`);
	console.log(
